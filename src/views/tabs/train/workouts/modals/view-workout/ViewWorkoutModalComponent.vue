<template>
    <div class="workout-modal-container">
        <div class="workout-outer-div">
            ...
            <div class="workout-exercise-div"  v-for="(exercise, exerciseIndex) in sessionWorkout.exercises" :key="exercise.id">
                <div class="exercise-header">
                    <div class="exercise-name">
                        {{ exercise.name }} 
                        <ion-icon :class="exercise.success ? 'show' : ''" :icon="checkmarkOutline" />
                    </div>
                ...
                <div class="exercise-rep-count-container">
                    <div class="exercise-rep-div" v-for="(set, index) in exercise.sets" :key="set.id">
                        <div class="exercise-rep-count" :class="set.completed ? 'selected' : ''" v-on:click="handleRepClick($event, exercise, index, exerciseIndex)">
                            {{ set.reps }}
                        </div>
                        <div>{{ set.weight }}</div>
                    </div>
            ...
            <div class="workout-lower-details">
                ...
                <div class="workout-finish-button" @click="submitWorkout()">FINISH WORKOUT</div>
                <div class="workout-modifications">
                    <div class="workout-add-exercise">ADD EXERCISE</div>
                    <div class="workout-edit-exercises">EDIT EXERCISES</div>
                </div>
            </div>
        </div>
    </div>
...
import { ellipsisHorizontal, chevronBackOutline, checkmarkOutline, ellipsisVertical } from 'ionicons/icons';
...
export default defineComponent({
    ...
    methods: {
        ...
        checkSetSuccess(event, exercise, index, exerciseIndex) {
            const originalReps = this.workout.exercises[exerciseIndex].sets[index].reps
...
<style>
    * {
    ...
    .lifted-amount, .lifted-label,
    .duration-amount, .duration-label {
        text-align: center;
    }
    .lifted-amount, .duration-amount {
        margin-bottom: 5px;
        font-weight: 900;
    }
